(* demo.sml - exercise the entity manager and two typed component stores.
...
fun pList xs = "[" ^ String.concatWith ", " (List.map pInt xs) ^ "]"
...
val () = print ("  created entities = " ^ pList [e0, e1, e2] ^ "\n")
val () = print ("  alive            = " ^ pList (EntityManager.alive m3) ^ "\n")
...
(* Remove a component, then destroy an entity (its id is recycled) *)
...
val () = print ("  alive = " ^ pList (EntityManager.alive m4) ^ "\n")
val (e3, _) = EntityManager.create m4
val () = print ("  next create reuses id = " ^ pInt e3 ^ "\n")
